...
- 管理员后台支持账号等级、Worker 优先级、队列可视化、安全事件、账号限制与申诉、关联账号额度和系统发信配置
...
对经管理员确认存在关联关系的普通账号，可以启用共享额度，避免通过多账号绕过限制；仅进入人工复核的关联不会自行改变既有用户额度。
...
- 限制账号会撤销现有会话、停止 CEAC/GTS/IRCC/韩国签证自动任务、终止排队任务，并在落库和发信前丢弃在途查询结果
- 被限制账号的档案数据会保留，注册邮箱会收到中性通知；用户可在受限页面提交申诉，恢复访问不会悄悄重新开启监控
